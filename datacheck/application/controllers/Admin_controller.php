...
class Admin_controller extends CI_Controller {
    ...
    public function product_doc($status = '') {
        $this->validSession();
        $this->load->view('top');
        $this->load->view('sidebar');
        $data['sectionName'] = 'Product Doc';
        $tableName = 'product_doc';
        $data['thisPath'] = SITE_PATH . 'product_doc/';
        ...
        $sqlDataArr['listing'] = $this->admin_model->getProductDocData($sqlData);

         //echo "<pre>";print_r( $sqlDataArr['listing']->result()); die;
                
        $data['paginationHTML'] = '';
        if ($sqlDataArr['listing']->num_rows() > 0) {
            $HTML = '';
            $sqlData['perPage'] = 'pagi';
            $rowCount = $this->admin_model->getProductDocData($sqlData);
            $rowCount = getPagination($rowCount);

            $data['paginationHTML'] = $this->getPaginationHTML($rowCount, $sqlData['selectTab']);
            //echo "<pre>"; print_r($sqlDataArr['listing']->result()); die;
            foreach ($sqlDataArr['listing']->result() as $list) {
                  //$gallery_image= explode('_', $list->gallery_image);
                $HTML .= '<tr class="gradeA even">';
               /* $HTML .= '<td>' . $list->id . '</td>';*/
                $HTML .= '<td>' . $list->name . '</td>';
                 $HTML .= '<td>' . $list->gallery_image . '</td>';
              /* if ($list->gallery_image) {
                    $image = '<img src="' . SITE_PRODUCTS_IMAGES_PATH . $list->gallery_image . '" width="100" >';
                } else {
                    $image = '';
                ...
                $HTML .= '</span>';
                $HTML .= '&nbsp;&nbsp;';
                $HTML .= '<a title="EDIT" href="' . SITE_PATH . 'addProductDoc/' . $list->id . '" >' . EDIT_ICON . '</a>';
                $HTML .= '&nbsp;&nbsp;&nbsp;&nbsp';
                $HTML .= '<a title="Delete" href="javascript:void(0)" onclick=statusChange("' . $tableName . '","-1","' . $list->id . '")>' . DELETE_ICON . '</a>';
                $HTML .= '</td>';
                $HTML .= '</tr>';
        ...
        $this->load->view('product_doc', $data);
        $this->load->view('footer');
    }

    //Add product Doc 
     public function addProductDoc($id = '') {
        $this->validSession();
        $this->load->view('top');
        $this->load->view('sidebar');
        $tableName = 'product_doc';
        $tableName2 = 'products';
        if ($id > 0) {
            $data['sectionName'] = 'Edit Product Doc';
        } else {
            $data['sectionName'] = 'Add  Product Doc';
        }
        $form_data['id'] = $id;
       // $form_data['road_id'] = '';   
       ...
       // $form_data['position'] = '';
        ...
        if ($id > 0) {
            ...
            if ($result) {
              ...
              //  $form_data['status'] = $result[0]->status;
                ...
                $form_data['id'] = $result[0]->id;
                $form_data['product_id']=$result[0]->product_id;
                $form_data['hiddenimage'] = $result[0]->gallery_image;
                //$form_data['description'] = $result[0]->description;
                $form_data['status'] = $result[0]->status;
            }
        ...
        if ($this->input->post('action') == "Submit") {
            $data['form_data'] = $this->input->post();
            $form_data['product_id'] = $this->input->post('product_id'); 
           // $form_data['description'] = $this->input->post('description');           
            if ($_FILES['image']['name'] != '') {
                $fileTmpLoc = $_FILES['image']['tmp_name'];
                $form_data['gallery_image'] = make_valid_image_name($_FILES['image']['name']);
            ...
            if ($id > 0) {
                unset($form_data['hiddenimage']);
               // echo $tableName; print_r($form_data) ; die; 
                $this->admin_model->updateData($tableName, $form_data, array("id" => $id));
                redirect('product_doc');
            } else {
                unset($form_data['hiddenimage']);
                $id = $this->admin_model->insertData($tableName, $form_data);
                if ($id > 0) {
        ...
        $this->data = new stdClass();
       // $this->data->roadmap = $this->admin_model->getData($tableName2);
         $this->data->productDocs = $this->admin_model->getData($tableName2);

        $form_data['productDocs'] = $this->data;
        $data['form_data'] = $form_data;
       //echo "<pre>"; print_r( $data['form_data']); die;
        
        $this->load->view('addProductDoc', $data);
        $this->load->view('footer');
    }

    // Product Video 

     public function product_video($status = '') {
        $this->validSession();
        $this->load->view('top');
        $this->load->view('sidebar');
        $data['sectionName'] = 'Product Video';
        $tableName = 'product_video';
        $data['thisPath'] = SITE_PATH . 'product_video/';
        ...
        $sqlDataArr['listing'] = $this->admin_model->getProductVideoData($sqlData);
        ...
        if ($sqlDataArr['listing']->num_rows() > 0) {
            $HTML = '';
            $sqlData['perPage'] = 'pagi';
            $rowCount = $this->admin_model->getProductVideoData($sqlData);
            $rowCount = getPagination($rowCount);
            ...
            foreach ($sqlDataArr['listing']->result() as $list) {
              ...
              /*if ($list->gallery_image) {
                ...
                $HTML .= '</span>';
                $HTML .= '&nbsp;&nbsp;';
                $HTML .= '<a title="EDIT" href="' . SITE_PATH . 'addProductVideo/' . $list->id . '" >' . EDIT_ICON . '</a>';
                $HTML .= '&nbsp;&nbsp;&nbsp;&nbsp';
                $HTML .= '<a title="Delete" href="javascript:void(0)" onclick=statusChange("' . $tableName . '","-1","' . $list->id . '")>' . DELETE_ICON . '</a>';
                $HTML .= '</td>';
                $HTML .= '</tr>';
    ...
    //Add product Video
     public function addProductVideo($id = '') {
        $this->validSession();
        $this->load->view('top');
        $this->load->view('sidebar');
        $tableName = 'product_video';
        $tableName2 = 'products';
        if ($id > 0) {
            $data['sectionName'] = 'Edit Product Video';
        } else {
            $data['sectionName'] = 'Add  Product Video';
        }
        $form_data['id'] = $id;
       // $form_data['road_id'] = '';   
        $form_data['product_id'] = '';  
      ...
      //  echo "<pre>"; print_r($form_data );     die;
       ...
       // $this->data->roadmap = $this->admin_model->getData($tableName2);
        ...
        $this->load->view('addProductVideo', $data);
        $this->load->view('footer');
    }
    ...
    public function gallery($id = '') {
       $this->validSession();
        $this->load->view('top');
        $this->load->view('sidebar');
        $data['sectionName'] = 'Gallery';
        $tableName = 'gallery';
        $data['thisPath'] = SITE_PATH . 'gallery/';
        ...
        $sqlDataArr['listing'] = $this->admin_model->getGalleryData($sqlData);
        ...
        if ($sqlDataArr['listing']->num_rows() > 0) {
            $HTML = '';
            $sqlData['perPage'] = 'pagi';
            $rowCount = $this->admin_model->getGalleryData($sqlData);
            $rowCount = getPagination($rowCount);
            ...
            foreach ($sqlDataArr['listing']->result() as $list) {
              ...
              if ($list->gallery_image) {
                ...
                $HTML .= '</span>';
                $HTML .= '&nbsp;&nbsp;';
                $HTML .= '<a title="EDIT" href="' . SITE_PATH . 'addGallery/' . $list->id . '" >' . EDIT_ICON . '</a>';
                $HTML .= '&nbsp;&nbsp;&nbsp;&nbsp';
                $HTML .= '<a title="Delete" href="javascript:void(0)" onclick=statusChange("' . $tableName . '","-1","' . $list->id . '")>' . DELETE_ICON . '</a>';
                $HTML .= '</td>';
                $HTML .= '</tr>';
    ...
    //Add Gallery
     public function addGallery($id = '') {
        ...
        //$tableName2 = 'products';
        if ($id > 0) {
            $data['sectionName'] = 'Edit Gallery';
        } else {
            $data['sectionName'] = 'Add  Gallery';
        }
        $form_data['id'] = $id;
       // $form_data['road_id'] = '';   
        $form_data['name'] = '';  
        ...
        if ($id > 0) {
            ...
            if ($result) {
               ...
               /* $form_data['road_id'] = $result[0]->road_id;
                ...
                $form_data['name'] = $result[0]->name;
                $form_data['gallery_image'] = $result[0]->gallery_image;
                $form_data['hiddenimage'] = $result[0]->gallery_image;
                $form_data['status'] = $result[0]->status;
            }
        }
      ...
      //  echo "<pre>"; print_r($form_data );     die;
        ...
        if ($this->input->post('action') == "Submit") {
            ...
            $form_data['name'] = $this->input->post('name'); 
           // $form_data['description'] = $this->input->post('description');  
            // $form_data['image'] = $this->input->post('image');           
            if ($_FILES['image']['name'] != '') {
                $fileTmpLoc = $_FILES['image']['tmp_name'];
                $form_data['gallery_image'] = make_valid_image_name($_FILES['image']['name']);
                $pathAndName = SERVER_PRODUCTS_IMAGES_PATH . $form_data['gallery_image'];
    ...
    public function gallery_image($id = '') {
        ...
        $this->load->view('top');
        $this->load->view('sidebar');
        $data['sectionName'] = 'Gallery Image';
        $tableName = 'gallery_image';
        $data['thisPath'] = SITE_PATH . 'gallery_image/';
        ...
        $sqlDataArr['listing'] = $this->admin_model->getGalleryImageData($sqlData);
        ...
        if ($sqlDataArr['listing']->num_rows() > 0) {
            $HTML = '';
            $sqlData['perPage'] = 'pagi';
            $rowCount = $this->admin_model->getGalleryImageData($sqlData);
            $rowCount = getPagination($rowCount);
           ...
           // echo "<pre>"; print_r($sqlDataArr['listing']->result()); die;
            foreach ($sqlDataArr['listing']->result() as $list) {
             ...
             if ($list->gallery_image) {
                ...
                $HTML .= '</span>';
                $HTML .= '&nbsp;&nbsp;';
                $HTML .= '<a title="EDIT" href="' . SITE_PATH . 'addGalleryImage/' . $list->id . '" >' . EDIT_ICON . '</a>';
                $HTML .= '&nbsp;&nbsp;&nbsp;&nbsp';
                $HTML .= '<a title="Delete" href="javascript:void(0)" onclick=statusChange("' . $tableName . '","-1","' . $list->id . '")>' . DELETE_ICON . '</a>';
                $HTML .= '</td>';
                $HTML .= '</tr>';
    ...
    //Add Gallery Image
     public function addGalleryImage($id = '') {
        ...
        $tableName2 = 'gallery';
        if ($id > 0) {
            $data['sectionName'] = 'Edit Gallery Image';
        } else {
            $data['sectionName'] = 'Add  Gallery Image';
        }
        $form_data['id'] = $id;
       // $form_data['road_id'] = '';   
        $form_data['gallery_id'] = '';  
        // $form_data['image'] = '';    
        $form_data['gallery_image'] = '';
         $form_data['description'] = '';
         $form_data['hiddenimage'] = '';
        ...
        if ($id > 0) {
            ...
            if ($result) {
               ...
               /* $form_data['road_id'] = $result[0]->road_id;
                ...
                $form_data['gallery_id'] = $result[0]->gallery_id;
                $form_data['description'] = $result[0]->description;
                 $form_data['gallery_image'] = $result[0]->gallery_image;
                $form_data['hiddenimage'] = $result[0]->gallery_image;
                $form_data['status'] = $result[0]->status;
      ...
      // echo "<pre>"; print_r($form_data );     die;
        ...
        if ($this->input->post('action') == "Submit") {
            $data['form_data'] = $this->input->post();
            $form_data['gallery_id'] = $this->input->post('gallery_id'); 
            $form_data['description'] = $this->input->post('description');  
            // $form_data['image'] = $this->input->post('image');           
            if ($_FILES['image']['name'] != '') {
                $fileTmpLoc = $_FILES['image']['tmp_name'];
            ...
            if ($id > 0) {
              //  echo "up"; die;
                unset($form_data['hiddenimage']);
                $this->admin_model->updateData($tableName, $form_data, array("id" => $id));
                redirect('gallery_image');
            } else {
                unset($form_data['hiddenimage']);
              // echo $tableName; print_r($form_data);  echo "in"; die;
                $id = $this->admin_model->insertData($tableName, $form_data);
                if ($id > 0) {
                    redirect('gallery_image');
                }
        ...
        $this->data = new stdClass();
       // $this->data->roadmap = $this->admin_model->getData($tableName2);
         $this->data->galleryImages = $this->admin_model->getData($tableName2);
