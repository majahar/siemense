<?php
	...
	$con = new connection();
		...
		$sql = "SELECT * from channel_partner order by id " ;
		
		/* Setup vars for query for pagination. */
		$andurl = "";
		$targetpage = PATH_REGISTER."form.php";
...
<div class="container">
                <div class="wrapper">
                    <?php include("../top.php"); 
					$sql = "SELECT * from channel_partner order by id desc";
         ...
         <div class="filter" >
         	<?php if(sizeof($results) > 0)
       		{
           	?>
			<!--<a class="btn blue" href="<?php echo PATH_REGISTER;?>download_report.php" id="Append_ID"> <?php if(isset($_GET['dateval'])) { ?>Download Report Of: <?php } else { ?>Download Report <?php } ?></a>-->
            <?php }?>
           ...
           <table  align="left"  style=" margin:auto" >
                <tr class="even">
                    ...
                    <th>Audited financial statement 2014-15</th>
                 	<th>Audited financial statement 203-14</th>
                  	<th>Audited financial statement 2012-13</th>
                    <th>Date</th>
                ...
                <tr class="<?php echo $class;?>">
                    <td><?php echo $i; ?></td>
                    <td><?php echo decode_str($result['name']); ?></td>
                    <td><?php echo decode_str($result['email']); ?></td>
                    <td><?php echo decode_str($result['mobile_no']); ?></td>
                    <td><?php echo decode_str($result['company']); ?></td>
                    <td><?php echo decode_str($result['city']); ?></td>
                    <td><?php echo decode_str($result['current_business']); ?></td>
                    <td><?php echo decode_str($result['designation']); ?></td>
                     
                    <td>
                    <?php
                    if($result['latest_cibil_score']!=''){?>
                    <a href="<?php echo USER_DOC.decode_str($result['latest_cibil_score']); ?>" target="_blank">Click Here</a>
                    <?php } ?>
                    </td>
                    
                     <td>
                     <?php
                      if($result['audited2014']!=''){?>
                     <a href="<?php echo USER_DOC.decode_str($result['audited2014']); ?>"target="_blank">Click Here</a>
                     <?php } ?>
                     </td>
                     
                      <td>
                      <?php
                      if($result['audited2013']!=''){?>
                      <a href="<?php echo USER_DOC.decode_str($result['audited2013']); ?>"target="_blank">Click Here</a>
                       <?php } ?>
                      </td>
                      
                       <td>
                      <?php
                      if($result['audited2012']!=''){?>
                      <a href="<?php echo USER_DOC.decode_str($result['audited2012']); ?>"target="_blank">Click Here</a>
                       <?php } ?>
                      </td>
                      
                      
                     <td><?php echo decode_str($result['added_on']); ?></td>
